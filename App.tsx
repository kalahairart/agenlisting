...
const App: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuthError(e.message || "Login gagal. Cek kredensial Anda.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDeleteVilla = async (id: string) => {
    if (!confirm('Hapus villa ini secara permanen?')) return;
    setIsLoading(true);
    try {
      ...
      <div className="min-h-screen bg-indigo-950 flex flex-col items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[2rem] shadow-2xl overflow-hidden p-8 md:p-12 animate-fadeIn">
          <div className="text-center mb-10">
            ...
            <h1 className="text-3xl font-extrabold text-gray-900">VillaPro Admin</h1>
            <p className="text-gray-500 mt-2">Masuk ke Dashboard Properti</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input name="email" type="email" required className="w-full pl-12 pr-4 py-3.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="agent@email.com" />
              </div>
            </div>
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input name="password" type="password" required className="w-full pl-12 pr-4 py-3.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="••••••••" />
              </div>
            </div>
            <button 
              type="submit" 
          ...
          <button 
            ...
            className="w-full mt-6 flex items-center justify-center gap-2 text-indigo-600 text-sm font-bold"
          >
            <DatabaseIcon size={16} /> Config Database
          </button>
        </div>
        ...
        <div className="space-y-8 animate-fadeIn">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1">Mengelola properti dari Supabase.</p>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-12 pr-4 py-4 rounded-xl border-none shadow-sm outline-none bg-white focus:ring-2 focus:ring-indigo-500" />
            </div>
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="px-6 py-4 rounded-xl border-none shadow-sm bg-white font-medium">
          ...
          {villas.length === 0 && !isLoading ? (
            <div className="text-center py-24 bg-white rounded-3xl border border-dashed border-gray-200">
              <Home className="text-gray-300 w-12 h-12 mx-auto mb-4" />
              <p className="text-gray-400 font-medium">Database kosong. Tambahkan villa pertama!</p>
            </div>
          ) : (
